...
$video = get_post_meta($post->ID, 'cake_video_embed' ,true);
?>
...
<div id="post-<?php the_ID(); ?>" <?php esc_attr(sanitize_html_class(post_class('article-post'))); ?>>
	...
	<div class="video-post">
		<?php
		if (cake_is_youtube($video)) {  
		echo '<div class="video-container"><a href="'.esc_url($video).'"  data-rel="youtube"></a></div>';
		} elseif (cake_is_vimeo($video)) {
		echo '<div class="video-container"><a href="'.esc_url($video).'"  data-rel="vimeo"></a></div>';
		}
		?>
	</div>
	<?php }	?>
