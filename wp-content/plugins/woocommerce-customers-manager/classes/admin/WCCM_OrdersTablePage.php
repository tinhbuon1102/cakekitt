		...
		if ( $column == 'wccm-user-details-page' ) 
		{
			...
			if($order->get_user_id( ) !=0)
				  echo '<a target="_blank" class="" href="'.admin_url().'?page=woocommerce-customers-manager&customer='.$order->get_user_id( ).'&action=customer_details">'.
					...
					'</a>';
			else echo '<a target="_blank" href="'.admin_url().'?page=woocommerce-customers-manager&customer_email='.$order->billing_email.'&action=customer_details"><span class="dashicons dashicons-admin-users"></span></a>';
		}
