...
/* include WCCM_PLUGIN_PATH.'Countries.php'; */
	...
	var $is_guest_customer;
	 ...
	 function __construct()
	 {
		...
		if( !isset($_POST['end_date_submit']) && !isset($_POST['start_date_submit']) ||  ($_POST['end_date_submit'] == '' && $_POST['end_date_submit'] ==''))
		{
			...
			$this->starting_date = date('Y/m/d', strtotime('-1 year'));
	 ...
	 {
		 global $wccm_wpml_helper, $wccm_customer_model;
	...
	?>
		<div id="user-general-details">
			...
			<p>
				...
				<label><?php  _e('Email Address', 'woocommerce-customers-manager'); ?></label><?php echo $this->customer_info->user_email ?> <br/>
				<label><?php  _e('Registration Date', 'woocommerce-customers-manager'); ?> </label> <?php echo $this->customer_info->user_registered ?> <br/>
				...
				<label><?php  _e('Total Spent', 'woocommerce-customers-manager'); ?> </label> <b><?php echo WCCM_CustomerDetails::get_user_total_spent(null,null, $orders); ?></b> <br/><br/>
			...
			<?php global $wpuef_htmlHelper, $wccm_product_model;
				...
				<div id="notes-box">
					...
					<p>
						<form name="customer_notes" method="post" action="">
							...
							<textarea style="clear:both" cols="40" rows="8" name="customer_notes"><?php if(isset($this->customer_extra_info['wccm_customer_notes'])) echo $this->customer_extra_info['wccm_customer_notes'][0];?></textarea>
							<p class="submit">
			...
			<div id="orders-list">
				<h3><?php _e('Orders', 'woocommerce-customers-manager'); ?> (<?php _e('period: ','woocommerce-customers-manager'); echo $this->starting_date. " - ".$this->ending_date; ?>)</h3>
				<p><strong><?php _e('NOTE', 'woocommerce-customers-manager'); ?>: </strong><?php _e('To display orders for a different period, select a different date range in the stat section.', 'woocommerce-customers-manager'); ?> </p>
				<?php 
					...
					<div class="order-block">
						...
						<?php
							...
							?>
									<tr>
										...
										<td> <?php echo get_woocommerce_currency_symbol().round($product['line_total'],2); ?> </td>
										<td> <?php echo get_woocommerce_currency_symbol().round($product['line_tax'],2); ?> </td>
						   ...
						   $order_total += $product['line_total'];
						   $taxes_total += $product['line_tax'];
						...
						<?php 
							$order_status = strtoupper($wc_order->status);
						...
						if(method_exists ($wc_order,'get_total_shipping'))
						{
						 ...
						 $total_order = get_woocommerce_currency_symbol().(round($order_total+$taxes_total+$wc_order->get_total_shipping()+$wc_order->get_shipping_tax()-$wc_order->get_total_discount() - $refounded,2));
						...
						else
						{
							...
							$total_order = get_woocommerce_currency_symbol().(round($order_total+$taxes_total-$wc_order->get_total_discount(),1));
						}
						...
						<span class="stats"><span class="stats_title"><?php _e('Discount:', 'woocommerce-customers-manager' ); ?></span> <span class="stats_content"><?php echo get_woocommerce_currency_symbol().round($wc_order->get_total_discount(),2); ?></span></span>
						...
						<span class="stats"><span class="stats_title"><?php _e('Payment method:', 'woocommerce-customers-manager' ); ?></span> <span class="stats_content"><?php echo $wc_order->payment_method_title; ?></span></span>
						<?php 
						//WCST support
		...
		<div id="user-geo-details-and-stats">
					...
					<h3> <?php _e('Send an email', 'woocommerce-customers-manager' ); ?></h3>
						<p>
							<form id="wccm-send-email-form" name="send-mail-form" method="post" action="">
								...
								<p><?php  _e('use <strong>{first_name}</strong>, <strong>{last_name}</strong>, <strong>{billing_first_name}</strong>, <strong>{billing_last_name}</strong>, <strong>{shipping_first_name}</strong>, <strong>{shipping_last_name}</strong> placeholders to render user first and last name in the email body.', 'woocommerce-customers-manager'); ?></p>
				...
				jQuery(document).ready(function()
				{
					...
					var radarChart = new Chart(ctx).Radar(radar_chart_data, {responsive : true});
					...
					var barChart = new Chart(ctx).Bar(bar_chart_data, {responsive : true});
					...
					var myLineChart = new Chart(ctx).Line(line_chart_data, {responsive : true}); 
