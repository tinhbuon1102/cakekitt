...
class WCCM_CustomerTable extends WP_List_Table {
    ...
    function column_login($item){
        ...
        //Build row actions
         $actions = array(
            'edit'      => sprintf('<a href="?page=%s&action=%s&customer=%s&edit=1">'.__('Edit', 'woocommerce').'</a>', $_REQUEST['page'], 'wccm-customer-add', $item['ID']),
            'delete'    => sprintf('<a style="color:red;" href="?page=%s&action=%s&customer=%s" onclick="return confirm(\''.__('Are you sure?','woocommerce-customers-manager').'\')" >'.__('Delete', 'woocommerce').'</a>',$_REQUEST['page'], 'delete' ,$item['ID']),
        );
       ...
       /*  return sprintf('%1$s <span style="color:silver">(id:%2$s)</span>%3$s',
		...
		//return '<a href="?page='.$_REQUEST['page'].'&customer='.$item['ID'].'&action=customer_details">'.$item['login'].'</a>';
		 return sprintf( get_avatar($item['ID'], 32, "", false, array('class'=>'wccm_avatar_img')).' <a href="?page='.$_REQUEST['page'].'&customer='.$item['ID'].'&action=customer_details">'.$item['login'].'</a>%1$s',
						$this->row_actions($actions));
    }
	...
	private function render_roles_switcher_dropdown()
	{
		...
		<div id="wccm_role_switcher_select_box">
			<label for="filter-by-date" class=" ">
				<?php _e( 'Assign new roles to your customers by selecting one or more from the following menu, select the customer from list and then clicking the Assign button.<br/><br/><strong><i>NOTE: Assigning new roles will overwrite old roles</i></strong>.', 'woocommerce-customers-manager'); ?>
			</label>
			<br/>
   ...
   function months_and_roles_dropdown( $post_type = 'shop_order' ) 
   {
	  global  $wp_locale, $wccm_order_model;
		...
		?>
			<div id="wccm-warning-roles-box" style="display: none;">
			<?php 
			...
			<a href="<?php echo get_admin_url(); ?>admin.php?page=wccm-options-page"><?php _e('To change displayed roles, go to Options menu.','woocommerce-customers-manager'); ?></a>
			</div>
				...
				*/
				if($can_print_user)
				{
					...
					if ( !empty( $customer_info->roles ) && is_array( $customer_info->roles ) ) 
						...
						{
							//$wp_roles->roles[$role_code]["name"]
							$roles .= $counter_roles_temp > 0 ? ",<br/>".$wp_roles->roles[$role_code]["name"] : $wp_roles->roles[$role_code]["name"]/* ." ("."<i>".$role_code."</i>)<br/>" */;
