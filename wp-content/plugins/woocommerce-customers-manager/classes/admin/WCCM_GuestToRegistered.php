...
class WCCM_GuestToRegistered
{
	...
	public function wccm_convert_guest_to_registered()
	{
		...
		$this->convert_guest_to_registered_by_order_id($order, $send_email );
		echo $this->conversion_result;
		wp_die();
	}
	public function convert_guest_to_registered_by_order_id($order, $send_email )
	{
		...
		$this->conversion_result = $conversion_result['conversion_result'];
		//change orders users id reference
		$wccm_order_model->update_order_customer_id($user['id'], $order);
		//send notification email
		if($send_email && !$user['already_exists'] && isset($order->billing_email))
		{
			$mail = new WCCM_Email();
			...
			$mail->trigger($order->billing_email, $subject,$text, 'guest_to_restered');
		}
	}
	public function woocommerce_metaboxes()
	{
		add_meta_box( 'woocommerce-customers-manager', __('Guest to registered customer conversion', 'woocommerce-customers-manager'), array( &$this, 'woocommerce_guest_to_registered_conversion_box' ), 'shop_order', 'side', 'high');
	}
	function woocommerce_guest_to_registered_conversion_box($post) 
	{
		...
		<div id="wccm-conversion-box">
			...
			<br/>			
			<select type="checkbox" id="wccm-conversion-notification-email" value="true">
			...
			<p><?php _e('The plugin will convert guest user to registered.<strong>In case the billing email used by the guest is already associated to a register user, the guest orders will be assigned to that registered user</strong>', 'woocommerce-customers-manager');?></p>
			<br/><br/>
		...
		<h3><?php _e('Customer successful converted!', 'woocommerce-customers-manager') ?></h3>
