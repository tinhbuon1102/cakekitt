...
{
	$user_id = email_exists($data_source->billing_email);
	$conversion_result = "";
	if($user_id != false)
	{
		$user_id = get_user_by( 'email', $data_source->billing_email );
		$user = array();
		$user['id'] = $user_id->ID;
		$user['already_exists'] = true;
		$conversion_result =  sprintf (__('Customer for %s email already existed. Customers have been merged', 'woocommerce-customers-manager'), $data_source->billing_email);
		return $user;
	}
	/* do
	{
	...
	$login = $data_source->billing_email;
	$exists = username_exists( $login );
	while($exists)
	{
		$chars_to_remove = array("'", "\"", " ", ":", "\\", "/", "*", "&");
	...
	$email = $data_source->billing_email;
	$userdata = array(
					'user_login'  =>  $login,
					'user_pass'   =>  $password,
					'user_email' => $email,
					'first_name' => isset($data_source->billing_first_name) ? $data_source->billing_first_name  : "", 
					'last_name' => isset($data_source->billing_last_name) ? $data_source->billing_last_name : "",
					'role' => $role
				);
	$user_id = wp_insert_user($userdata); 
	//wp_set_password( $user[ 'Password' ], $user_id); ;			
		
	if(isset($data_source->billing_first_name))
		update_user_meta( $user_id, 'billing_first_name', $data_source->billing_first_name );
	if(isset($data_source->billing_last_name))
		update_user_meta( $user_id, 'billing_last_name', $data_source->billing_last_name );
	if(isset($email))
		update_user_meta( $user_id, 'billing_email', $email );
	if(isset($data_source->billing_phone))
		update_user_meta( $user_id, 'billing_phone', $data_source->billing_phone );
	if(isset($data_source->billing_company))
		update_user_meta( $user_id, 'billing_company', $data_source->billing_company );
	if(isset($data_source->billing_address_1))
		update_user_meta( $user_id, 'billing_address_1', $data_source->billing_address_1 );
	if(isset($data_source->billing_address_2))
		update_user_meta( $user_id, 'billing_address_2', $data_source->billing_address_2 );
	if(isset($data_source->billing_postcode))
		update_user_meta( $user_id, 'billing_postcode', $data_source->billing_postcode );
	if(isset($data_source->billing_city))
		update_user_meta( $user_id, 'billing_city', $data_source->billing_city );
	if(isset($data_source->billing_state))
		update_user_meta( $user_id, 'billing_state', $data_source->billing_state );
	if(isset($data_source->billing_country))
		update_user_meta( $user_id, 'billing_country', $data_source->billing_country );
	
	if(isset($data_source->shipping_first_name))	
		update_user_meta( $user_id, 'shipping_first_name', $data_source->shipping_first_name );
	if(isset($data_source->shipping_last_name))
		update_user_meta( $user_id, 'shipping_last_name', $data_source->shipping_last_name );
	if(isset($data_source->shipping_phone))
		update_user_meta( $user_id, 'shipping_phone', $data_source->shipping_phone );
	if(isset($data_source->shipping_company))
		update_user_meta( $user_id, 'shipping_company', $data_source->shipping_company );
	if(isset($data_source->shipping_address_1))
		update_user_meta( $user_id, 'shipping_address_1', $data_source->shipping_address_1 );
	if(isset($data_source->shipping_address_2))
		update_user_meta( $user_id, 'shipping_address_2', $data_source->shipping_address_2 );
	if(isset($data_source->shipping_postcode))
		update_user_meta( $user_id, 'shipping_postcode', $data_source->shipping_postcode );
	if(isset($data_source->shipping_city))
		update_user_meta( $user_id, 'shipping_city', $data_source->shipping_city );
	if(isset($data_source->shipping_state))
		update_user_meta( $user_id, 'shipping_state', $data_source->shipping_state );
	if(isset($data_source->shipping_country))
		update_user_meta( $user_id, 'shipping_country', $data_source->shipping_country );
			...
			foreach($orders as $order)
			{
				...
				if($order_data)
				{
					...
					if(!isset($customers[$user]))
					{
						...
						$customers[$user]['last_order_date'] = $order->post_date;
						$customers[$user]['orders'] = 0;					
						$customers[$user]['total_spent'] = 0; //11
						$customers[$user]['billing_name'] =  str_replace($bad_chars, '',$order_data['_billing_first_name']); //20
						...
						$customers[$user]['billing_phone'] =  $order_data['_billing_phone']; //12
						...
						$customers[$user]['shipping_name'] =  str_replace($bad_chars, '',$order_data['_shipping_first_name']); //20
						...
						if($get_last_order_id)
							$customers[$user]['last_order_id'] =  $order->ID;//29
	...
	public function set_wpuef_field_content($field_id, $value, $user_id )
	{
		...
		if(function_exists('wpuef_set_field'))
		{
			...
			else if(isset($result->field_type) && $result->field_type == "date" ) 
			{
				...
				wpuef_set_field($field_id, $date->format("Y/m/d"), $user_id);
			}
