			...
			jQuery.each(guest_customers, function( key, value_array )
				{
					...
					if(typeof value_array !== 'string')
					{
						...
						html_string += '<td><input type="checkbox" class="guest-checkbox" data-customer-email="'+value_array[5]+'"></input><span class="hidden_data">'+JSON.stringify( value_array )+'</span></td>';
						html_string += '<td class="conversion column-conversion"><button data-lastorderid="'+value_array[33]+'" class="conversion-button">Guest to Registered</button><input class="wccm_email_notification" type="checkbox" value="yes" checked="checked"><span class="wccm_email_notification_label">Send an email with login?</span></input>';
						...
						html_string +=		'</div></td>';
						html_string += '<td class="name column-name"> <a href="?page=woocommerce-customers-manager&customer_email='+value_array[5]+'&action=customer_details" target="_blank">'+value_array[2]+'</a></td>';
