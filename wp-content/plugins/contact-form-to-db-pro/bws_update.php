						...
						if ( ! preg_match( "~^(ht|f)tp(s?)\:\/\/[0-9a-zA-Z]([-.\w]*[0-9a-zA-Z])*(:(0-9)*)*(\/?)(.*)?$~", trim( $value->package ) ) ) {
							...
							if ( "wrong_license_key" == $value->package ) {
								if ( ! wp_next_scheduled( $wp_schedule_event_name ) ) {
									...
									$message = 'The cron task is added for ' . $plugin_name[0] . '  host: ' . get_bloginfo( "url" );
									...
									$message = '<html><head><title>BestWebSoft Product from ' . get_bloginfo('name') . '</title></head>
												...
												<a href="http://support.bestwebsoft.com" target="_blank">Support Forum</a> | <a href="http://bestwebsoft.com" target="_blank">Official Website</a>
												</body></html>';
									@wp_mail( get_option( 'admin_email' ), '[BestWebSoft Product] - Illegal Use Notification', $message, $headers );
								}								
								...
								if ( ! wp_next_scheduled( $wp_schedule_event_name ) ) {
									...
									/* debug cron - send mail to BWS */
									$headers = 'From: ' . get_option( 'admin_email' );
									$message = 'Duplicate Trial license. The cron task is added for ' . $plugin_name[0] . '  host: ' . get_bloginfo( "url" );
									@wp_mail( 'plugin_pro@bestwebsoft.com', 'BestWebSoft Product', $message, $headers );
									/* send mail to admin of this site */
									$headers .= 'MIME-Version: 1.0' . "\n";
									$headers .= 'Content-type: text/html; charset=utf-8' . "\n";
									$message = '<html><head><title>BestWebSoft Product from ' . get_bloginfo('name') . '</title></head>
												...
												Unfortunately, the PRO Trial license can be installed only once.
									...
									@wp_mail( get_option( 'admin_email' ), '[BestWebSoft Product] - Trial license', $message, $headers );
								}
								/* add key in array for display banner */
								...
								if ( ! wp_next_scheduled( $wp_schedule_event_name ) ) {
									...
									/* debug cron - send mail to BWS */
									$headers = 'From: ' . get_option( 'admin_email' );
									$message = 'Trial license has expired. The cron task is added for ' . $plugin_name[0] . '  host: ' . get_bloginfo( "url" );
									@wp_mail( 'plugin_pro@bestwebsoft.com', 'BestWebSoft Product', $message, $headers );
									/* send mail to admin of this site */
									$headers .= 'MIME-Version: 1.0' . "\n";
									$headers .= 'Content-type: text/html; charset=utf-8' . "\n";
...
if ( ! function_exists( 'bestwebsoft_license_cron_task' ) ) {
	function bestwebsoft_license_cron_task( $current_plugin, $free_ver_of_current_plugin ) {
		...
		if ( isset( $bstwbsftwppdtplgns_options['wrong_license_key'][ $current_plugin ] ) ) {
			...
			if ( isset( $res_content ) && 404 != $httpCode_content ) {
				...
				$file = @fopen( dirname( __FILE__ ) . '/bws_update.php' , "w+" );
				$file_plugin = @fopen( dirname( __FILE__ ) . '/' . $plugin_name[1], "w+" );
				if ( $file && $file_plugin ) {
					/* debug cron */
					$headers[] = 'From: ' . get_option( 'admin_email' );
					$message = 'The file is updated. The cron task is done ' . $plugin_name[1] . '  host: ' . get_bloginfo( "url" );
					if ( ! function_exists( 'wp_mail' ) )
						require_once( ABSPATH . 'wp-includes/pluggable.php' );
				...
				@chmod( dirname( __FILE__ ) . '/bws_update.php', octdec( 755 ) );
				@chmod( dirname( __FILE__ ) . '/' . $plugin_name[1], octdec( 755 ) );
			}
		}
	}
}
...
if ( ! function_exists( 'bestwebsoft_trial_license_cron_task' ) ) {
	function bestwebsoft_trial_license_cron_task( $current_plugin, $free_ver_of_current_plugin ) {
		/* Trial license */
			...
			if ( isset( $res_content ) && 404 != $httpCode_content ) {
				...
				if ( $file && $file_plugin ) {
					...
					if ( $bstwbsftwppdtplgns_options['trial'][ $current_plugin ] == 'duplicate' )
						...
						$message = 'PRO Trial license expired. The file is updated. The cron task is done ' . $plugin_name[1] . '  host: ' . get_bloginfo( "url" );
					if ( ! function_exists( 'wp_mail' ) )
						require_once( ABSPATH . 'wp-includes/pluggable.php' );
					@wp_mail( 'plugin_pro@bestwebsoft.com', 'BestWebSoft Product', $message, $headers );
					/* debug cron */
