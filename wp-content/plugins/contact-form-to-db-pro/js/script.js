(function($) {
	$(document).ready( function() {
		...
		*/
		...
		$( '#cntctfrmtdb_save_attachments' ).change( function() {
			if ( $( this ).is( ':checked' ) ) {
				$( '.cntctfrmtdb_save_to_block' ).css( 'display', 'block' );
			} else {
				$( '.cntctfrmtdb_save_to_block' ).css( 'display', 'none' );
			}
		});		
		$( '#cntctfrmtdb_delete_messages' ).change( function() {
			if ( $( this ).is( ':checked' ) ) {
				$( '.cntctfrmtdb_delete_block' ).css( 'display', 'table-cell' );
				$( '#cntctfrmtdb_delete_messages_after' ).removeAttr( "disabled" );
			} else {
				$( '.cntctfrmtdb_delete_block' ).css( 'display', 'none' );
				$( '#cntctfrmtdb_delete_messages_after' ).attr('disabled', 'disabled');
			}
		});
							...
							success: function( result ) { 
								...
								$( '.check-column input:checkbox' ).each( function() {
									if ( $( this ).val() == messageId ) {
										...
										This is necessary to correctly counted by javascript new height of '.message-container' - block and slide down him in full height. */
										setTimeout( function() { 
		...
		*/
		if ( $.isFunction( $.fn.fancybox ) ) {
			$( 'a.attachment-fancybox' ).fancybox({
				'transitionIn'      : 'fade',
				'transitionOut'     : 'fade',
				'speedIn'           : 1000, 
				'speedOut'          : 1000 
		...
		/* network settings */
		if ( $( 'input[name="cntctfrmtdb_network_apply"]:checked' ).val() != 'all' ) {
			$( '.cntctfrmtdb_network_apply_all' ).hide();
			$( '#cntctfrmtdb_network_notice' ).hide();
			if ( $( 'input[name="cntctfrmtdb_network_apply"]:checked' ).val() == 'off' ) {
				$( '#cntctfrmtdb_settings_form tr' ).hide();
				$( '#cntctfrmtdb_settings_form tr.cntctfrmtdb_network_settings' ).show();
			}
		} else {
			$( '#cntctfrmtdb_network_notice' ).show();
			$( '.cntctfrmtdb_network_apply_all' ).show();
		}
		$( 'input[name="cntctfrmtdb_network_apply"]' ).change( function() {
			if ( $( this ).val() != 'all' ) {
				$( '.cntctfrmtdb_network_apply_all' ).hide();
				$( '#cntctfrmtdb_network_notice' ).hide();
				if ( $( 'input[name="cntctfrmtdb_network_apply"]:checked' ).val() == 'off' ) {
					...
					$( '#cntctfrmtdb_settings_form tr' ).show();
			} else {
				$( '#cntctfrmtdb_network_notice' ).show();
				$( '.cntctfrmtdb_network_apply_all' ).show();
				$( '#cntctfrmtdb_settings_form tr' ).show();
...
function cntctfrmtdb_change_read_status( messageId ) {
	( function($) {
		$.ajax({
			url: ajaxurl,
			type: "POST",
			data: { action: "cntctfrmtdb_read_message", cntctfrmtdb_ajax_read_status: 1, cntctfrmtdb_ajax_message_id: messageId, 'cntctfrmtdb_nonce' : cntctfrmtdb.ajax_nonce },
	...
	( function($) {
		if ( oldStatus != newStatus ) {
			$.ajax({
				url: ajaxurl,
				type: "POST",
				data: { action: "cntctfrmtdb_change_staus", cntctfrmtdb_ajax_message_status: newStatus, cntctfrmtdb_ajax_message_id: messageID, cntctfrmtdb_ajax_old_status: oldStatus, 'cntctfrmtdb_nonce' : cntctfrmtdb.ajax_nonce },
				success: function( result ) {
					$( '.updated' ).hide();
