...
class SCFP_FormEntries extends Agp_Module {
    ...
    public function entrieColumns( $columns ){
        ...
        foreach( $fields as $key => $value ):
            if (!empty( $value['visibility'] ) && $value['field_type'] !== 'captcha') :
    ...
    public function fillEntrieColumns( $column ){
        ...
        switch ( $column ) {
            ...
            default :
                $fields = SCFP()->getSettings()->getFieldsSettings();        
                foreach( $fields as $key => $value ):
                    if ($key == $column && !empty( $value['visibility'] ) && $value['field_type'] !== 'captcha') :
                        if (in_array( $value['field_type'], array( 'checkbox', 'radio', 'select' ) ) ) {
