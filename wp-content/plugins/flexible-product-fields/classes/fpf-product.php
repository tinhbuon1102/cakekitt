...
class FPF_Product {
    ...
    public function add_to_cart_text( $text ) {
        ...
        if ( $product instanceof WC_Product &&  !is_single() ) {
            ...
            if ( ! $product_extended_info->is_type_grouped() && $this->product_has_required_field( $product ) ) {
    ...
    public function create_field( array $field, WC_Product $product ) {
        ...
        if ( $field_type['has_price'] ) {
            ...
            if ( isset( $field['price_type'] ) && $field['price_type'] != '' && isset( $field['price'] ) && $field['price'] != '' ) {
	            ...
	            $args['label'] .= ' <span id="' . $key . '_price">(' . $price . ')</span>';
            }
        }
                ...
                foreach ( $field['options'] as $option ) {
	                ...
	                if ( $field_type['has_price_in_options'] ) {
                        if ( isset( $option['price_type'] ) && $option['price_type'] != '' && isset( $option['price'] ) && $option['price'] != '' ) {
	                        ...
	                        $price                       = $this->wc_price( $price );
	                        if ( $field['type'] == 'radio' ) {
		                        $options[ $option['value'] ] .= ' <span id="' . $key . '_' . $option['value'] . '_price">(' . $price . ')</span>';
	                        }
	                        else {
        ...
        if ( $field['type'] == 'heading' ) {
            $ret = $this->_plugin->load_template( 'heading', 'fields', array( 'title' => wpdesk__( $field['title'], 'flexible-product-fields' ), 'type' => $field['type'], 'args' => $args, 'key' => $key ) );
        }
